				...
				<section>
					<div class="container">
						...
						<div class="content__wrapper plusone" style="padding: 11px 0 10px;">
							<?php
								$i = 0;
							  	$showMore = count($recent_post->posts) > 4;
								if ($recent_post->have_posts()) {
									while ($recent_post->have_posts() && $i < 4): $recent_post->the_post();
										$postexc = get_the_excerpt();
										?>
										<?php if ($i!=0): ?>
									...
									endwhile; ?>
									<?php if ($showMore): ?>
										<button type="button" name="button" class="btn btn--red btn--decoration btn--large btn--center btn--show-more show-more-top" data-page="taxonomy"><?php _e('SHOW MORE', 'alabama-policy-institute'); ?><img src="img/icons/ico--arrow--down--red.png" alt="" class="btn__icon"></button>				
									<?php endif ?>
									<?php
									wp_reset_postdata();
								}
				...
				<section >
					<div class="container">
						...
						<div class="content__wrapper">
							<?php
								$i = 0;
							  	$showMore = count($taxonomy_posts->posts) > 4;
								if ($taxonomy_posts->have_posts()) {
									while ($taxonomy_posts->have_posts() && $i < 4): $taxonomy_posts->the_post();
										$postexc = get_the_excerpt();
										?>
										<?php if ($i==0): ?>
									...
									endwhile; ?>
									<?php if ($showMore ): ?>
										<button type="button" name="button" class="btn btn--red btn--decoration btn--large btn--center btn--show-more" data-page="multimedia"><?php _e('SHOW MORE', 'alabama-policy-institute'); ?><img src="img/icons/ico--arrow--down--red.png" alt="" class="btn__icon"></button>
									<?php endif ?>
									<?php
									wp_reset_postdata();
								}
