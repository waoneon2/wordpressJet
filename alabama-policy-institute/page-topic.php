<?php
/* Template Name: Topic
 * Description: A Page Template created by ChaidirWeb.Com.
 */
?>
...
<?php
    ...
    $current_taxonomy = get_query_var( 'topic' );
    ...
    <div id="primary" class="content-area">
        <main id="main" class="site-main">
                <section class="featured-img hero hero--small hero--list hero--topic-landing" style="background-image: url('<?php echo get_template_directory_uri() ?>/img/backgrounds/bg-topic-detail--hero.jpg')">
                    <h2 class="hero__heading"><?php _e('Topic', 'alabama-policy-institute'); ?></h2>
                            ...
                            <?php
                                ...
                                if ($taxonomy_posts->have_posts()) {
                                    while ($taxonomy_posts->have_posts() && $i <= 9): $taxonomy_posts->the_post();
                                        $terms_obj = wp_get_post_terms($taxonomy_posts->post->ID, 'topic');
                                        ?>
                                                ...
                                                <div class="content__inner">
                                                    ...
                                                    <p class="p-type">
                                                        <?php foreach ($terms_obj as $k => $v): ?>
                                                            <a href="<?php echo get_term_link($v, 'topic') ;?>">,
                                                                <?php echo $v->name ?></a>
                                                        <?php endforeach ?>
                                                    </p>
                                    ...
                                    endwhile; ?>
                                    <?php if ($showMore ): ?>
                                        <button type="button" name="button" class="btn btn--red btn--decoration btn--large btn--center btn--show-more2" data-page="topic"><span><?php _e('SHOW MORE', 'alabama-policy-institute'); ?></span><img src="<?php echo get_template_directory_uri() ?>/img/icons/ico--arrow--down--red.png" alt="" class="btn__icon"></button>
                                    <?php endif ?>
                                    <?php
                                    wp_reset_postdata();
                                }
