 ...
 */
class Jetty_Widget_Andeavor_Category_Update extends WP_Widget
{
	/**
	...
	{
		parent::__construct(
			'jetty_widget_andeavor_category_update',
			__('Jetty Widget - Andeavor Category Update', 'jetty'),
	...
	public function widget( $args, $instance )
	{
			...
			$title = apply_filters('widget_title', empty($instance['title']) ? 'Latest Updates:' : $instance['title'], $instance, $this->id_base);
								...
								if ( $wp_query->have_posts() ) :
									...
									?>
										<li <?php echo ($i > 2) ? 'class=""' : ''; ?>>
											<a href="<?php the_permalink(); ?>"><span class="title"><?php echo get_the_title()?></span>
											<span class="content-datetime"><?php echo get_the_time('(gA l, F n)')?></span></a>
										</li>
									<?php
									$i++;
									endwhile;
