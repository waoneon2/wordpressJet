(function (doc, $, i18n) {
  ...
  function createMediaItem(wid, index) {
    ...
    attachment.name = 'widget-jetty_irotator_widget[' + wid + '][media][' + index + '][image]'
    attachment.className = 'jirw_repeatable_attachment_id_field'
    attachment.value = -1

    chooser.className = 'jetty-image-choose-action'

    button.href = '#'
    button.className = 'jirw_upload_file_button button button-primary'
  ...
  function fileUploadHandler(e) {
    ...
    imageinput     = target.closest('div.media-item').find('.jirw_repeatable_attachment_id_field'),
  ...
  function addMoreHandler(e) {
    ...
    var container   = findAncestor(e.currentTarget, 'jetty-irotator-widget-form'),
  ...
  function removeImageHandler(e) {
    ...
    var box        = findAncestor(e.currentTarget, 'media-item'),
        container  = findAncestor(e.currentTarget, 'jetty-irotator-widget-form')

    container.removeChild(box);
    var items = container.querySelectorAll('.media-item')
    if (items.length < 8) {
      var button = container.querySelector('.jirw-add-more')
  ...
  function main() {
    $(doc.body)
      .on('click', '.jetty-irotator-widget-form .jirw_upload_file_button', fileUploadHandler)
      .on('click', '.jetty-irotator-widget-form .jirw-add-more', addMoreHandler)
      .on('click', '.jetty-irotator-widget-form .jetty-remove-image', removeImageHandler)
  }
  // load on first animation frame, effectively it wait documents body ready
  requestAnimationFrame(main)
})(document, jQuery, jirw_admin_uploader_i18n)
