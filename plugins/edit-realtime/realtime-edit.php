...
{
    if (in_array(get_current_screen()->post_type, ['post', 'page'], true)) {
...
{
    if (in_array($p->post_type, ['post', 'page'], true)) {
        return false;
    }
...
function jte_admin_enqueue_scripts()
{
    ...
    if ($pagenow === 'post-new.php' || $pagenow === 'post.php') {
        if (in_array($typenow, ['post', 'page'], true)) {
            // disable autosave
            wp_dequeue_script('autosave');
            // register and enqueue it
            if (!wp_script_is('pusher', 'registered')) {
                wp_register_script('pusher', '//js.pusher.com/2.2/pusher.min.js', ['jquery'], '2.2', true);
...
function jte_admin_print_enqueue_scripts()
{
    ...
    $script .= "\nvar JTEC = ".wp_json_encode([
        'current_uid' => TextOperation\random_str(32),
    ]).';';
    $script .= "</script>\n";
    echo $script;
...
{
    $edits = get_posts(['post_type' => 'editing', 'post_status' => 'inherit']);
    if (count($edits) > 0) {
        ...
        switch ($wp->query_vars['jte-pusher-route']) {
            ...
            case '/auth':
                if (is_user_logged_in() && $_SERVER['REQUEST_METHOD'] === 'POST') {
                    global $current_user;
                    get_currentuserinfo();
                    $presence_data = ['name' => $current_user->display_name];
