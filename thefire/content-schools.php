...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
    ...
    <div class="entry-content">
        ...
        <div class="entry-thumbnail">
            <?php the_post_thumbnail('school-logo'); ?>
        </div>
        <?php endif; ?>     
        ...
        <header class="entry-header">Speech Code Rating</header> 
        <p>
        <?php 
			...
			if ( $connectedCases->have_posts() ) {
				...
				<ul class="no-style">
					<?php while ( $connectedCases->have_posts() ) : $connectedCases->the_post();
						...
						if($categories[0]->cat_ID == 531 || $categories[0]->cat_ID == 532 || $categories[0]->cat_ID == 533 || $categories[0]->cat_ID == 820) {
						?>
			...
			if ( $connectedCommentary->have_posts() ) {
				...
				<ul class="no-style">
					<?php while ( $connectedCommentary->have_posts() ) : $connectedCommentary->the_post(); 
						...
						if($categories[0]->cat_ID == 8 || in_array($categories[0]->cat_ID, $sub_categories)) {
						?>
							<li><?php get_template_part( 'content-cases_tabs', get_post_format() ); ?></li>
							<?php 
							$countConnectedCommentary++;
						} 
					endwhile; ?>
                </ul>
