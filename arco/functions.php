<?php
/**
 * arco functions and definitions.
 *
...
function arco_setup() {
	 ...
	 * If you're building a theme based on arco, use a find and replace
	 * to change 'arco' to the name of your theme in all the template files.
	 */
	load_theme_textdomain( 'arco', get_template_directory() . '/languages' );
	...
	register_nav_menus( array(
		'primary' => esc_html__( 'Primary', 'arco' ),
	) );
	...
	add_theme_support( 'custom-background', apply_filters( 'arco_custom_background_args', array(
		'default-color' => 'ffffff',
		'default-image' => '',
	) ) );
}
endif;
add_action( 'after_setup_theme', 'arco_setup' );
 ...
 */
function arco_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'arco_content_width', 640 );
}
add_action( 'after_setup_theme', 'arco_content_width', 0 );
 ...
 */
function arco_widgets_init() {
	register_sidebar( array(
		'name'          => esc_html__( 'Sidebar', 'arco' ),
		'id'            => 'sidebar-1',
		'description'   => '',
...
add_action( 'widgets_init', 'arco_widgets_init' );

/**
 * Enqueue scripts and styles.
 */
function arco_scripts() {
	wp_enqueue_style( 'font-awesome', get_template_directory_uri() . '/css/font-awesome.css' );

	wp_enqueue_style( 'arco-style', get_stylesheet_uri() );

	wp_enqueue_style( 'grid-style', get_template_directory_uri() . '/css/grid.css' );

	wp_enqueue_script( 'arco-navigation', get_template_directory_uri() . '/js/navigation.js', array('jquery'), '20120206', true );

	wp_enqueue_script( 'arco-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20130115', true );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
}
add_action( 'wp_enqueue_scripts', 'arco_scripts' );
    ...
    if ( !is_front_page() ) {
        ...
        if ( is_archive() && !is_tax() && !is_category() && !is_tag() ) {
            ...
            if($post_type != 'post') {
                ...
                $post_type_name = $post_type_object->labels->name == 'Sheets' ? __('Fact Sheets') : $post_type_object->labels->name;
            ...
            printf( esc_html__( 'Search Results for: %s', 'arco' ), '<span class="bread-current bread-current-' . get_search_query() . '" title="Search results for: ' . get_search_query() . '">' . get_search_query() . '</span>');
...
add_action('after_setup_theme', 'arco_remove_formats', 100);
function arco_remove_formats()
{
