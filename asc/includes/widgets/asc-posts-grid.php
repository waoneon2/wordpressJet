...
/***** ASC Posts Grid (Homepage) *****/

class asc_posts_grid extends WP_Widget {
    function __construct() {
		parent::__construct(
			'asc_posts_grid', esc_html_x('ASC Posts Grid (Homepage)', 'widget name', 'asc'),
			array(
				'classname' => 'asc_posts_grid',
				'description' => esc_html__('Display posts as grid on front page for use in "Home 1" or "Home 4" widget areas.', 'asc'),
    ...
    function widget($args, $instance) {
        ...
        echo $before_widget;
			...
			<div class="asc-fp-grid-widget clearfix"><?php
				while ($widget_loop->have_posts()) : $widget_loop->the_post();
					if ($counter == 1) {
						echo '<div class="asc-section asc-group">' . "\n";
					}
					if ($counter >= 1) {
					...
					if ($counter % 4 == 0 && $counter != $postcount) {
						echo '</div>' . "\n";
						echo '<hr class="asc-separator hidden-sm">' . "\n";
						echo '<div class="asc-section asc-group">' . "\n";
					}
					$counter++;
				endwhile;
