...
class asc_facebook_page extends WP_Widget {
	function __construct() {
		parent::__construct(
			'asc_facebook_page', esc_html_x('ASC Facebook Page', 'widget name', 'asc'),
			array('classname' => 'asc_facebook_page', 'description' => esc_html__('Widget to display your Facebook page on your website.', 'asc'))
		);
	}
    function widget($args, $instance) {
    	$defaults = array('title' => esc_html__('Follow on Facebook', 'asc'), 'fb_url' => 'https://www.facebook.com/MHthemes', 'width' => 373, 'height' => 500, 'cover' => 0, 'faces' => 1, 'posts' => 0);
    ...
    function form($instance) {
    	$defaults = array('title' => esc_html__('Follow on Facebook', 'asc'), 'fb_url' => 'https://www.facebook.com/ASCthemes', 'width' => 373, 'height' => 500, 'cover' => 0, 'faces' => 1, 'posts' => 0);
        $instance = wp_parse_args($instance, $defaults); ?>
        ...
        <p>
        	<label for="<?php echo esc_attr($this->get_field_id('fb_url')); ?>"><?php esc_html_e('Facebook Page URL:', 'asc'); ?></label>
       	...
       	<p>
	    	<label for="<?php echo esc_attr($this->get_field_id('width')); ?>"><?php esc_html_e('Width (min. 280px):', 'asc'); ?></label>
	    ...
	    <p>
	    	<label for="<?php echo esc_attr($this->get_field_id('height')); ?>"><?php esc_html_e('Height (min. 130px):', 'asc'); ?></label>
		...
		<p>
			...
			<label for="<?php echo esc_attr($this->get_field_id('cover')); ?>"><?php esc_html_e('Hide cover photo', 'asc'); ?></label>
		</p>
		<p>
			...
			<label for="<?php echo esc_attr($this->get_field_id('faces')); ?>"><?php esc_html_e('Show profile photos', 'asc'); ?></label>
		</p>
		<p>
			...
			<label for="<?php echo esc_attr($this->get_field_id('posts')); ?>"><?php esc_html_e('Show posts from FB page', 'asc'); ?></label>
		</p><?php
    }
}
