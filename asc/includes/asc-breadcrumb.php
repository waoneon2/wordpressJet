...
if (!function_exists('asc_breadcrumbs')) {
	function asc_breadcrumbs() {
		if (!is_home() && !is_front_page()) {
			global $post;
			$asc_options = asc_theme_options();
			if ($asc_options['breadcrumbs'] == 'enable') {
				$delimiter = ' ';
				...
				$close_span = '</span>';
				echo '<nav class="breadcrumb">' . $before_link . '<a href="' . esc_url(home_url()) . '" itemprop="url" class="bc-home">' . $before_title . esc_html__('Home', 'asc') . $close_span . '</a>' . $close_span . $delimiter;
					if (is_single() && get_post_type() == 'post' && !is_attachment()) {
						...
						echo $before_text . esc_html__('Media', 'asc') . $close_span;
					} elseif (is_page()) {
						if ($post->post_parent) {
						...
						echo $before_text . esc_html__('Authors', 'asc') . $close_span;
					} elseif (is_404()) {
						echo $before_text . esc_html__('Page not found (404)', 'asc') . $close_span;
					} elseif (is_search()) {
						echo $before_text . esc_html__('Search', 'asc') . $close_span;
					} elseif (is_date()) {
						...
						if (is_day()) {
							echo $before_link . '<a href="' . $url_year . '" title="' . esc_html__('Yearly Archives', 'asc') . '" itemprop="url">' . $before_title . $arc_year . $close_span . '</a>' . $close_span . $delimiter;
							echo $before_link . '<a href="' . $url_month . '" title="' . esc_html__('Monthly Archives', 'asc') . '" itemprop="url">' . $before_title . $arc_month . $close_span . '</a>' . $close_span . $delimiter . $before_text . $arc_day . $close_span;
						} elseif (is_month()) {
							echo $before_link . '<a href="' . $url_year . '" title="' . esc_html__('Yearly Archives', 'asc') . '" itemprop="url">' . $before_title . $arc_year . $close_span . '</a>' . $close_span . $delimiter . $before_text . $arc_month . $close_span;
						} elseif (is_year()) {
...
add_action('asc_before_post_content', 'asc_breadcrumbs');
add_action('asc_before_page_content', 'asc_breadcrumbs');
