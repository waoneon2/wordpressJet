(function($) {
		...
		jQuery.fn.gotopage = function(page) {
			...
			if (page==1) {
				...
				if (settings.pages[page].title) {
					$(id + '_nav .multipage_next').html(settings.pages[page].title);
				} else {
					$(id + '_nav .multipage_next').html('Next');
				}
	...
	jQuery.fn.validatePage = function(page, all) {
		if (all) return true
		var eventData = {
			validate: true,
			page: page
		}
		$(this).trigger('multipage:validate', [eventData])
		return eventData.validate
	}


	jQuery.fn.validateAll = function() {
		for (x = 1; x <= settings.pages.length; x++) {
			if (!$(this).validatePage(x, true)) {
				$(this).gotopage(x);
				return false;
			}
		}
	...
	jQuery.fn.multipage = function(options) {
		...
		settings = jQuery.extend({stayLinkable:false,submitLabel:'Submit',hideLegend:false,hideSubmit:true,generateNavigation:true,activeDot:'<div class="progress-point progress-point-full"><span></span></div>',inactiveDot:'<div class="progress-point"><span></span></div>'},options);
		id = '#' + $(this).attr('id');
		var form = $(this);
		...
		$(id +' fieldset').hide();
          ...
          $('<div class="multipage_nav" id="'+id_name+'_nav"><a href="#" class="multipage_next" onclick="return $(\''+id+'\').nextpage();">Next</a><div class="clearer"></div></div>').insertAfter(this);
				}
			}
