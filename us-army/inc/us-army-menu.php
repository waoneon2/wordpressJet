...
/**
 *
 * @package US ARMY MENU
 */

class us_army_walker_nav_menu extends Walker {
...
class us_army_walker_nav_menu_link extends Walker {
    var $tree_type = array( 'post_type', 'taxonomy', 'custom' );
    var $db_fields = array( 'parent' => 'menu_item_parent', 'id' => 'db_id' );
    var $count = 1;
    function start_lvl(&$output, $depth = 0, $args = array()) {
    ...
    function start_el(&$output, $object, $depth = 0, $args = array(), $current_object_id = 0) {
        ...
        $class_names = strlen( trim( $class_names ) ) > 0 ? ' class="' . esc_attr( $class_names )  .' col-md-3 box-link"' : '';
        $id = apply_filters( 'nav_menu_item_id', '', $object, $args );
        $id = strlen( $id ) ? ' id="' . esc_attr( $id ) . '"' : '';
        $output .= "\n$indent\t" .'<li' . $id . $value . $class_names .'>'."\n\t$indent\t";
        $attributes  = ! empty( $object->attr_title ) ? ' title="'  . esc_attr( $object->attr_title ) .'"' : '';
