...
function firecomps_setup() {
	...
	register_post_type( 'school', array(
		'labels' => array('name' => __( 'Schools' ), 'singular_name' => __( 'School' )),
		'public' => false,
		'show_ui' => true,
		'has_archive' => false,
		'hierarchical' => true,
...
function firecomps_scripts_styles() {
	...
	wp_enqueue_script( 'comm100-script', 'http://hosted.comm100.com/AdminPluginService/js/subscribe.js', array(), '2013-10-08');
	wp_enqueue_script( 'google-maps-v3', 'http://maps.google.com/maps/api/js?sensor=false', array(), '2013-10-14' );
...
<div class="item">
	...
	<?php if ($category_name): echo sprintf(__('<span>%s of the Month:</span>', 'firecomps'), $category_name); endif; ?>
	<?php the_title(); ?>
    </a></h1>
    <?php
	...
	if ( $categories_list ):
	?>
    <p class="categories-links"><?php echo $categories_list; ?></p>
    <?php endif; ?>
    <figure><a href="<?php the_permalink(); ?>"><img src="<?php echo get_post_meta(get_the_ID(), 'banner_home_image', true) ;?>" alt="<?php the_title(); ?>" /></a></figure>
</div>
<?php
	endwhile;
}
...
<section class="featured-content-home clearfix">
    ...
    <footer><a href="<?php echo get_category_link(12); ?>">Read More Featured Articles</a></footer>
</section>
<?php
	endif;
}
...
<section class="wrapper grid recent-home clearfix">
	<header><?php echo __('Recent', 'firecomps'); ?><span><?php echo __('Cases, press release, and commentary', 'firecomps'); ?></span></header>
    ...
    <?php endif; ?>
    <?php $i++; endwhile; ?>
    <footer><a href="<?php echo get_category_link(3); ?>"><?php echo __('View More', 'firecomps'); ?></a></footer>
</section>
<?php
	endif;
}
...
<section class="wrapper grid defend-home clearfix">
    ...
    <?php if ($i % 2 == 0): ?><div class="item grid-long-2x1"><?php endif; ?>
    	<div>
            <h1><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a></h1>
            <figure><a href="<?php the_permalink(); ?>"><img src="<?php echo get_post_meta(get_the_ID(), 'grid_home_image', true) ;?>" alt="<?php the_title(); ?>" /></a></figure>
        </div>
...
<section class="wrapper grid video-home clearfix">
	<header>
        ...
        <span><?php echo __('Watch the latest video about protecting freedom of speech.', 'firecomps'); ?></span>
    </header>
    <?php $i = 0; while ($query_video->have_posts()): $query_video->the_post(); $thumbnail = get_post_custom_values('video_thumbnail', get_the_ID()); ?>
    <?php if ($thumbnail): ?>
...
<section class="wrapper grid popular-posts clearfix">
	<header>
        ...
        <span><?php echo sprintf(__('Today\'s top %s from around the country', 'firecomps'), $cat_name); ?></span>
        <a href="<?php echo get_category_link($cat_id); ?>" class="more"><?php echo sprintf(__('More %s', 'firecomps'), $cat_name); ?></a>
    </header>
...
function firecomps_get_school_data($school_name) {

	// if (empty($school_name)) return null;
	// $slug = strtolower(preg_replace('/\s+/', '-', $school_name));
	// $args = array(
	// 	'name' => $slug, 
	// 	'post_type' => 'school',
	// 	'post_status' => 'publish',
...
function firecomps_ajax() {
	...
	if ($args) {
		...
		while ($query_school->have_posts()) {
			...
			if ($action == 'get_states_short') {
				$values = get_post_custom_values('iso_short_name', get_the_ID());
				if ($values) $data[] = $values[0];
			} else {
				$data[] = get_the_title();
			}
		}
